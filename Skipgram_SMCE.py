...
def softmaxCostAndGradient(v, u_index, outputVectors, dataset):
    ...
    ### YOUR CODE HERE
    ### Cost
    Y_pre = softmax( np.dot(outputVectors, v) ) # 1 x N/2  
    Y = np.zeros(Y_pre.shape)                # 1 x N/2 
    Y[u_index] = 1
    cost_temp = - np.log(Y_pre[u_index])
    ## Gradient :
    #  Calculate the grad for predictions(1 x dimVectors):
    gradv_temp = np.dot(outputVectors.T, (Y_pre - Y))
    #  Calculate the grad for outputVectors (N/2 x dimVectors):
    gradu_temp = np.outer((Y_pre - Y), v)
    ### END YOUR CODE
