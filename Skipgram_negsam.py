...
from q2_sigmoid_new import sigmoid
#########################################################################################
...
def negSamplingCostAndGradient(v, u_index, outputVectors, dataset, K=10):
    ...
    ### YOUR CODE HERE
    indices = [u_index]
    indices.extend(getNegativeSamples(u_index, dataset, K))
    ### Cost
    gradu_temp = np.zeros(outputVectors.shape)  # N/2 x nd
    gradv_temp = np.zeros(v.shape)  # 1 x nd

    u_temp = outputVectors[indices] #(K+1) x nd

    z = np.dot(u_temp, v) #1 x (K+1)

    z_temp = sigmoid ( z ) #1 x (K+1)
    z_temp[0] = z_temp[0] - 1

    z = - z
    z[0] = - z[0]
    cost_temp = - np.sum( np.log( sigmoid ( z ) ) )

    ## Gradient :
    #  Calculate the grad for predictions(1 x dimVectors):
    gradv_temp = np.dot(z_temp, u_temp) # 1 x nd
    #  Calculate the grad for outputVectors (N/2 x dimVectors):
    gradu = np.outer(z_temp, v)
    for i in range(len(indices)):
        gradu_temp[indices[i]] = gradu_temp[indices[i]] + gradu[i]
    ### END YOUR CODE
...
def gradcheck_naive(f, x):
    ...
    while not it.finished:
        ...
        if reldiff > 1e-5:
            ...
            assert print('Gradient check failed')
            return
